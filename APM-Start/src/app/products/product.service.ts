...
export class ProductService {
  ...
  selectedProduct$ = combineLatest([this.productsWithCategory$, this.productSelectedAction$]).pipe(
    tap(([products, selectedProductId]) => console.log(`products`, products,"/n selectedProductId: ", selectedProductId)),
  ...
  productsWithAdd$ = merge(this.productsWithCategory$, this.productInsertedAction$.pipe(shareReplay())).pipe(
  ...
  selectedProductSuppliers$ = this.selectedProduct$.pipe(
    ...
    // mergeMap((selectedProduct) => // may cause wrong data when selection changed quickly
    switchMap((selectedProduct) =>
      from(selectedProduct.supplierIds).pipe(
        mergeMap(supplierId => this.http.get<Supplier>(`${this.suppliersUrl}/${supplierId}`)),
        toArray(), // colloct the emissions and emit them as a single array
